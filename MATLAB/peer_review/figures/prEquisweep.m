...
mode = strtrim(fileread('mode.txt'));
mode
switch mode
	case 'test'
		Nsamp  = 1e1;
...
for Nmeas=Nmvals
    ...
    param_str = sprintf('prFig1data_Nsamp%d_Nperm%d_c%d_Amp%d', Nsamp, Nperm, ...
                            round(cfact*100), Amp);
    out_dir   = fullfile(pwd, strcat(mode,'_',sprintf('results_%s_%s', param_str, ts)));
