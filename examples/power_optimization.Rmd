...
In this notebook we denomstrate how to perform an exhaustive search using PowerCHORD in R.
...
```{r}
db_fname = 'temp/cNecks_24_8.txt'

wlen     = 6 # half the original window length
bvec = exhaustiveSearch(N=N,
                 Nfine=Nfine,wlen=wlen,wdensity=wdensity,
                 returnType='optimal',db_fname=db_fname) meas_times = tau[bvec>0] evalWorstPower(meas_times,param=list(Amp=2,freq=1))
```
...
If you want to see the entire database, you can turn off the window cosntraint as follows
```{r}
...
wdensity = 1   # perscribed number of measurements during window
df = exhaustiveSearch(N=N,
                 Nfine=Nfine,wlen=wlen,wdensity=wdensity,
                 returnType='all',db_fname=db_fname)
nrow(df) # number of constrained solutions
