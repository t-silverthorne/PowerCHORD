...
//------------------------------------------------------
void usage() {
  printf("Usage: necklace [type] [n] [k] [max] [d | m forbid_substring | content] (1<=type<=12 or 21<=type<=27, n>=1, k>=2)\n");
}
//--------------------------------------------------------------------------------
int main(int argc, char *argv[]) {
  ...
  if ((type < 1) || ((type > 12) && (type < 21)) || (type > 27) || (N < 1) || (K < 2)) { usage();  return 1; }
  ...
  // 10. Chord diagrams
  // 11. Lyndon brackets (basis for n-th homogeneous component of free Lie algebra)
  // 12. De Bruijn cycle
  ...
  if (CONTENT) {
    i = 1;
    sum = 0;
    if (argc != 5+K) { printf("invalid content\n");  return 1; }
    for (j=1; j<=K; j++) {
    ...
    if (sum != N) { printf("invalid content\n"); return 1; }
    K = i-1;
  }
  //---------------------------------------
  if (FORBIDDEN) {
    ...
    if (argc != 6+M) {  printf("invalid forbidden substring\n");  return 1; }
